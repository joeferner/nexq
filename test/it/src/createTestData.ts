  ...
  for (let i = 0; i < 30; i++) {
    ...
    for (let m = 0; m < Math.floor(Math.random() * 50); m++) {
      ...
      await client.api.sendMessage(queueName, {
        body: JSON.stringify({ message: 'this is a long test string to check display of large strings in the TUI interface when inspecting a message, we want to make sure it wraps correctly. (multi-wide character ⛩️⛩️⛩️⛩️⛩️⛩️⛩️⛩️)' }),
        attributes,
      });
    }
  }
